...
func solveMaze() {
	direction := make(chan amaze.Direction)
	survey := make(chan amaze.Survey)
	mr := amaze.NewMazeRunner()
	go mr.FindTreasure(survey, direction)
	survey <- amaze.Survey{Survey: awake()}
	for {
		select {
		case dir := <-direction:
			s, err := Move(string(dir))
