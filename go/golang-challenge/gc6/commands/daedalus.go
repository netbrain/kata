...
import (
	...
	"github.com/golangchallenge/gc6/amaze"
...
func createMaze() *Maze {
	realMaze := emptyMaze()
	m := amaze.NewMaze(realMaze.Width(), realMaze.Height())
	m.GenerateMaze()

	for x := 0; x < realMaze.Width(); x++ {
		for y := 0; y < realMaze.Height(); y++ {
			rr, _ := realMaze.GetRoom(x, y)
			r, _ := m.GetRoom(x, y)

			if r.Walls.Bottom {
				rr.AddWall(mazelib.S)
			}

			if r.Walls.Top {
				rr.AddWall(mazelib.N)
			}

			if r.Walls.Left {
				rr.AddWall(mazelib.W)
			}

			if r.Walls.Right {
				rr.AddWall(mazelib.E)
			}

			if r.Treasure {
				realMaze.SetTreasure(x, y)
			}

			if r.Start {
				realMaze.SetStartPoint(x, y)
			}
